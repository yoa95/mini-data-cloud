    ...
    <div className="flex items-end space-x-1" style={{ height }}>
      {data.slice(-20).map((point, index) => {
        const normalizedHeight =
          ((point.value - minValue) / range) * (height - 4);
        return (
          <div
            key={index}
            className="bg-blue-500 rounded-sm flex-1 min-w-[2px]"
            style={{ height: Math.max(2, normalizedHeight) }}
            title={`${point.value.toFixed(
              2
            )} at ${point.timestamp.toLocaleTimeString()}`}
          />
        );
      })}
    </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-blue-600">
                {Math.round(
                  mockMetrics.reduce(
                    (acc, m) => acc + Math.abs(m.trendPercentage),
                    0
                  ) / mockMetrics.length
                )}
                %
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-3">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {Math.abs(metric.trendPercentage).toFixed(1)}% in the last{" "}
                      {timeRange}
                    </div>
                  </div>
                  <Badge
            ...
            {mockMetrics.filter((m) => Math.abs(m.trendPercentage) > 10)
              .length === 0 && (
